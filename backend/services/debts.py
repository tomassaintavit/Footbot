...
def delete_debt_by_player_name(player_name: str):
    try:
        ...
        if player.get("dni"):
            result = sheets_sync.set_player_debt(player["dni"], 0)
        ...
        return {"success": True, "message": f"¡Listo! Se eliminaron las deudas de {player['name']}."}
...
def create_debt_by_player_name(player_name: str, amount: float):
    try:
        ...
        supabase.table("debts").insert({
            "player_id": player["id"], "amount": amount, "is_paid": False
        }).execute()
        return {"success": True, "message": f"✅ Se cargó una deuda de ${amount} para {player['name']}."}
